...
*   **Procesamiento de Pagos:** Integración fluida con la pasarela de pagos.
...
*   **Infrastructure:** Implementaciones técnicas como controladores REST, adaptadores de servicios externos y repositorios (Prisma/Postgres).
...
2.  **Configurar variables de entorno:**
    Copia el archivo `.env.example` a `.env` y completa las credenciales necesarias (DB, Keys).
    ```bash
    cp .env.example .env
    ```
...
*   **Firma de Integridad:** Implementación de firmas SHA-256 para asegurar la integridad de las transacciones.
...
3.  **Procesamiento:** Se envía la solicitud de pago a la pasarela con el token de tarjeta.
